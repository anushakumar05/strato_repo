                ...
                while insert_idx < len(content) and content[insert_idx].strip() and not content[insert_idx].strip().startswith('## '):
                    ...
                    if content[insert_idx].strip().startswith(entry_title_prefix):
                        ...
                        if entry_scan_match: # Check if it's a valid entry title line
                            # Advance past the current entry and its associated content
                            current_entry_start_idx = insert_idx
                            # Find the first line *after* the current entry title that is either a new header or blank
                            # This loop finds the end of the current entry's description
                            while insert_idx < len(content) and not content[insert_idx].strip().startswith('#') and content[insert_idx].strip():
                                insert_idx += 1
                            # insert_idx is now at a blank line, a new header, or end of file
                            continue # Re-evaluate from this new insert_idx
...
def update_readme_for_reverse_array_command():
    """Updates README.md with documentation for the 'reverse-array' command."""
    readme_path = "README.md"
    section_header = "## Commands"
    entry_prefix = "###"
    entry_markdown = """
### `reverse-array`
Reverses the order of elements in an array (list).
*   **Usage**: `python main.py reverse-array <element1> <element2> ...`
*   **Arguments**:
    *   `<elementN>`: Any number of elements to be reversed. Elements are treated as strings.
*   **Example**:
    *   `python main.py reverse-array apple banana cherry` returns `['cherry', 'banana', 'apple']`.
"""
    _update_readme_section(readme_path, section_header, entry_markdown, entry_prefix)
...
if __name__ == "__main__":
    ...
    try:
        ...
        update_readme_for_convert_command()
        update_readme_for_reverse_array_command() # New command documentation
        ...
        update_readme_for_celsius_to_fahrenheit_function()
        update_readme_for_check_number_sign_function()
