                ...
                while insert_idx < len(content) and content[insert_idx].strip() and not content[insert_idx].strip().startswith('## '):
                    ...
                    if content[insert_idx].strip().startswith(entry_title_prefix):
                        ...
                        if entry_scan_match: # Check if it's a valid entry title line
                            ...
                            insert_idx += 1
                            while insert_idx < len(content) and not content[insert_idx].strip().startswith('#') and not content[insert_idx].strip().startswith(entry_title_prefix):
                                insert_idx += 1
                            # insert_idx is now at the start of the next entry or the next section, or end of file
                            continue # Re-evaluate from new insert_idx
                    insert_idx += 1 # If not another entry or part of it, just move to next line
...
def update_readme_for_decimal_to_binary_function():
    """Updates README.md with documentation for the 'decimal_to_binary' function."""
    readme_path = "README.md"
    section_header = "## Functions"
    entry_prefix = "###"
    entry_markdown = """
...
*   **Inputs**: `decimal_num` (int): The non-negative decimal integer to convert.
*   **Outputs**: (str): The binary representation.
*   **Raises**: `TypeError` if input is not int, `ValueError` if input is negative.
*   **Example**: `decimal_to_binary(10)` returns `"1010"`.
"""
    _update_readme_section(readme_path, section_header, entry_markdown, entry_prefix)
...
if __name__ == "__main__":
    ...
    try:
        ...
        update_readme_for_decimal_to_binary_function() # NEW LINE
