                ...
                while insert_idx < len(content) and content[insert_idx].strip() and not content[insert_idx].strip().startswith('## '):
                    ...
                    if content[insert_idx].strip().startswith(entry_title_prefix):
                        ...
                        if entry_scan_match: # Check if it's a valid entry title line
                            # Advance past the current entry. Find its end.
...
def update_readme_for_decimal_to_binary():
...
*   **Inputs**: `decimal_num` (int): A non-negative decimal integer.
*   **Outputs**: (str): The binary representation as a string.
...
if __name__ == "__main__":
    ...
    try:
        ...
        update_readme_for_decimal_to_binary() # New function documentation
